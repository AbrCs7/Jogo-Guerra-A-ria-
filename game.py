import pygame      #Em python usamos hashtag para fazer comentário
import helicopter  #Vocês devem comentar o código desta forma
import enemy_heli
import boat
import sprites
import random
...
pygame.mixer.pre_init(44100, -16, 1, 512) #esses textos como "frequency:" são gerados
pygame.init()                                                     #automaticamente, não implemente eles.
    ...
    while menu:
        ...
        if cloud_x <= 800 - 1100:
            ...
            if not player.wreck_start:
                cloud_x -= 5
        if godmode:
            title = message_to_screen("GUERRA AERIA (GODMODE)", font, 80, yellow)
        else:
            title = message_to_screen("GUERRA AERIA", font, 100, black)
        controls_1 = message_to_screen("use W A S D para mover, SPACE para atirar,", font, 30, black)
        controls_2 = message_to_screen("SHIFT para soltar bombas e P para pausar", font, 30, black)
        if selected == "play":
        ...
        pygame.display.set_caption("Guerra Aéria a " + str(int(clock.get_fps())) + " frames/s  - Desenvolvido por Abraão Rodrigues")
        clock.tick(FPS)
...
def game_loop():
    ...
    while not game_exit:
        ...
        while game_over:
            ...
            your_score = message_to_screen("SEU SCORE FOI: " + str(score), font, 50, black)
            if game_over_selected == "play again":
            ...
            pygame.display.update()
            pygame.display.set_caption("Guerra Aéria a  " + str(int(clock.get_fps())) + " frames/s - Topicos Especiais para Eng. da Computacao")
            clock.tick(10)
